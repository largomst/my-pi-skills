...
```bash
python3 pocketbase/scripts/pb_client.py list_collections
```

**Get a specific collection:**
```bash
python3 pocketbase/scripts/pb_client.py get_collection --collection <collection_id_or_name>
```

**Create a collection:**
```bash
python3 pocketbase/scripts/pb_client.py create_collection --name <name> --type <base|auth|view> --schema '<json_schema>'
```
...
**List records:**
```bash
python3 pocketbase/scripts/pb_client.py list_records --collection <collection_name> --page 1 --per-page 30 --filter 'created > "2023-01-01"'
```

**Create a record:**
```bash
python3 pocketbase/scripts/pb_client.py create_record --collection <collection_name> --data '{"field": "value"}'
```

**Update a record:**
```bash
python3 pocketbase/scripts/pb_client.py update_record --collection <collection_name> --id <record_id> --data '{"field": "new_value"}'
```

**Delete a record:**
```bash
python3 pocketbase/scripts/pb_client.py delete_record --collection <collection_name> --id <record_id>
```
...
**Create a user:**
```bash
python3 pocketbase/scripts/pb_client.py create_user --data '{"email": "user@example.com", "password": "password123", "passwordConfirm": "password123", "name": "User Name"}'
```
...
**Backup database:**
```bash
python3 pocketbase/scripts/pb_client.py backup_database --name <backup_name>
```
